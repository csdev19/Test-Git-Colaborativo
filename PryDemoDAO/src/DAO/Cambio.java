...
public class Cambio {
    ...
    public List ListaTitles(Titles t) throws Exception {
        List lista = new ArrayList(); //Polimorfismo
        String sql = "select isbn, title, price from titles where publisherID = ?";
        Connection cn = cnBooksOracle();
        PreparedStatement pst = cn.prepareStatement(sql);
        
        pst.setInt(1, t.getPubID());
        ResultSet rs = pst.executeQuery();

        //Copiar el resultset a la coleccion
        
        while (rs.next()) {
            t = new Titles(rs.getString("isbn"), rs.getString("title"), 0, "", 0, "", rs.getDouble("price"));
            lista.add(t);
        }//while
